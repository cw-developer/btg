<?php if( get_row_layout() == 'three_column_title_section' ):
$html = '';
$title_box_1 = get_sub_field('title_box_1');
$title_box_2 = get_sub_field('title_box_2');
$image = get_sub_field('image');
$bottom_title_content = get_sub_field('bottom_title_content');
$bottom_content_left = get_sub_field('bottom_content_left');
$bottom_content_right = get_sub_field('bottom_content_right');
$customclass = get_sub_field('custom_class');
$html .= '<section class="three_column_title_section-block '.$customclass.'">';
$html .= '<div class="top_section"><div class="container-fluid"><div class="row top_row"><div class="col-lg-9 col-md-12 p-0">';
$html .= '<div class="title_boxes_wrap"><div class="title_content_wrap first_title_box">';
$html .= '<div class="breadcrumbs_wrap">'.get_breadcrumb().'</div>';
$html .= $title_box_1;
$html .= '</div>';
$html .= '<div class="title_content_wrap second_title_box content-color-Light">';
$html .= $title_box_2;
$html .= '</div>';
$html .= '</div></div>';
$html .= '<div class="col-lg-3 col-md-12 p-0"><div class="title_image_wrap">';
$html .= '<img src="'.$image['url'].'" title="'.$image['title'].'" alt="'.$image['alt'].'" width="'.$image['width'].'" height="'.$image['height'].'">';
$html .= '</div></div>';
$html .= '</div></div></div>';
$html .= '<div class="bottom_section"><div class="container"><div class="row bottom_row"><div class="col-md-12">';
$html .= '<div class="content_wrap">';
$html .= $bottom_title_content;
$html .= '</div>';
$html .= '<div class="bottom_content_wrap"><div class="content_wrap">';
$html .= $bottom_content_left;
$html .= '</div>';
$html .= '<div class="content_wrap">';
$html .= $bottom_content_right;
$html .= '</div></div>';
$html .= '</div></div></div></div>';
$html .= '</section>';
echo $html;
endif; ?>