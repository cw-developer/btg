...
class TRP_IN_SP_Slug_Manager {
            ...
            if (!empty($query_vars['name'])) {
                if (!empty($query_vars['post_type'])) {
                    ...
                    foreach( $postnames as $postname ){
                        /**
                         * For Hebrew language the characters are urlencoded in the wp_postmeta table but in $query_vars they come as decoded
                         * so in the function "get_original_slug" we send a parameter in Hebrew and the meta value is %d7%9e%d7%95%d7%a6%d7%a8 so
                         * we cannot identify it and the page for the Hebrew slug is not found resulting in 404 error. This does not seem to
                         * happen for other languages with special characters since they come in $query_vars urlencoded (tried with Arabic, Japanese, Chinese, Russian )
                         */
                        if ($TRP_LANGUAGE === 'he_IL'){
    ...
    public function filter_registration_args_for_slug( $args, $name ){
	    ...
	    if( is_admin() || $TRP_LANGUAGE === $this->settings['default-language'] || (method_exists('TRP_Translation_Manager', 'is_resp_api_request') && $this->translation_manager::is_rest_api_request() ))
		    return $args;
