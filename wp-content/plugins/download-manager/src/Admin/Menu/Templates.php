...
class Templates
{
    ...
    function __construct()
    {
        ...
        add_action('wp_ajax_wpdm_send_test_email', array($this, 'saveTestEmail'));
    ...
    {
        if(wp_verify_nonce(wpdm_query_var('__tenonce'), WPDM_PUB_NONCE)){
            check_ajax_referer(WPDM_PUB_NONCE, '__tenonce');
            ...
            if ( isset( $_REQUEST['template'] ) ) {
                $params['template_file'] = wpdm_query_var('template');
            }

            $id = wpdm_query_var('emailid', ['default' => 'user-signup']);
            Email::send( $id, $params );
            wp_send_json(['success' => true, 'message' => sprintf(esc_attr__( 'Test email sent to %s', WPDM_TEXT_DOMAIN ), $email), 'type' => 'success']);
        } else {
            wp_send_json(['success' => false, 'message' => esc_attr__( 'Session Expired! Refreah the page and try again.', WPDM_TEXT_DOMAIN ), 'type' => 'danger']);
        }
    }
