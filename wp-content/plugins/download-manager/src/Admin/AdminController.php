...
class AdminController {
    ...
    function enqueueScripts() {
        ...
        if (wpdm_query_var('post_type') === 'wpdmpro' || get_post_type() === 'wpdmpro' || in_array(wpdm_query_var('page'), $wpdm_pages) || ($pagenow == 'index.php' && wpdm_query_var('page') == '') || in_array($pagenow, $allow_bscss)) {
            ...
            wp_enqueue_style('wpdm-admin-styles', plugins_url('/download-manager/assets/css/admin-styles.min.css'), 9999);
    ...
    function metaBoxes() {
	    ...
	    if(current_user_can('upload_files')) {
		    $meta_boxes = array(
			    'wpdm-attached-files'      => array(
				    'title'    => __( "Attached Files", "download-manager" ),
				    'callback' => array( $this, 'Files' ),
				    'position' => 'normal',
				    'priority' => 'core'
			    ),
			    'wpdm-attached-dir'        => array(
				    'title'    => __( "Attach Dir", "download-manager" ),
				    'callback' => [ $ServerDirBrowser, 'metaBox' ],
				    'position' => 'side',
				    'priority' => 'core'
			    ),
			    'wpdm-settings'            => array(
			    ...
			    'wpdm-upload-file'         => array(
				    'title'    => __( "Attach File", "download-manager" ),
				    'callback' => array( $this, 'uploadFiles' ),
				    'position' => 'side',
				    'priority' => 'core'
			    ),
			    'wpdm-additional-previews' => array(
                    "title"    => __("Additional Previews", WPDM_TEXT_DOMAIN),
                    "callback" => array( $this, 'additionalPreviews' ),
                    "position" => "side",
                    "priority" => "core"
			    ),
		    );
