        ...
        <table id="wpdmmydls-<?php echo $table_id; ?>" class="table table-striped wpdm-all-packages-table">
            ...
            <tr>
                <?php foreach ($colheads as $ix => $colhead) {
                    ...
                    $width = (isset($_colhead[1])) ? "width: {$_colhead[1]} !important;min-width: {$_colhead[1]} !important;max-width: {$_colhead[1]} !important;" : "";
                    ?>
                    <th <?php echo $width ? "style='{$width}'" : ""; ?> id="<?php echo $cols[$ix][0]; ?>"
                        class="<?php if ($ix > 0) echo 'hidden-sm hidden-xs'; ?>"><?php _e($_colhead[0], 'download-manager'); ?></th>
                <?php } ?>
                ...
                if (WPDM()->package->userCanAccess($data['ID']) || !get_option("_wpdm_hide_all", 0)) {
                    ...
                    <tr class="__dt_row">
                        ...
                        foreach ($cols as $colx => $cold) {
                            ...
                            ?>
                            <td <?php if (in_array($cold[0], array('publish_date', 'update_date', 'create_date'))) { ?> data-order="<?php echo $dor[$cold[0]]; ?>" <?php } ?>
