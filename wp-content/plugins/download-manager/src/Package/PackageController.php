...
class PackageController extends PackageTemplate {
	...
	function prepare( $ID = null, $template = null, $template_type = 'page' ) {
		...
		if ( in_array( 'update_date', $template_tags ) ) {
			$post_vars['update_date'] = date_i18n( get_option( 'date_format' ), strtotime( $post_vars['post_modified'] ) + wpdm_tzoffset() );
		}
		...
		$post_vars['download_link'] = (int) get_option( '__wpdm_mask_dlink', 1 ) == 1 ? "<a class='wpdm-download-link download-on-click {$post_vars['btnclass']}' {$open_in_new_window} rel='nofollow' href='#' data-downloadurl=\"{$post_vars['download_url']}\">{$post_vars['link_label']}</a>" : "<a class='wpdm-download-link {$post_vars['btnclass']}' rel='nofollow' href='{$post_vars['download_url']}' {$open_in_new_window}>{$post_vars['link_label']}</a>";
		...
		if ( self::userDownloadLimitExceeded( $post_vars['ID'] ) ) {
		...
		else if ( isset( $post_vars['expire_date'] ) && $post_vars['expire_date'] != "" && $post_vars['expire_date_ts'] < time() ) {
			$post_vars['download_url']  = '#';
			$post_vars['link_label']    = __( "Download was expired on", "download-manager" ) . " " . date_i18n( get_option( 'date_format' ) . " h:i A", $post_vars['expire_date_ts'] + wpdm_tzoffset() );
		...
		else if ( isset( $post_vars['avail_date'] ) && $post_vars['avail_date'] != '' && $post_vars['avail_date_ts'] > time() ) {
			$post_vars['download_url']  = '#';
			$post_vars['link_label']    = __( "Download will be available from ", "download-manager" ) . " " . date_i18n( get_option( 'date_format' ) . " h:i A", $post_vars['avail_date_ts'] + wpdm_tzoffset() );
			$post_vars['download_link'] =
			$post_vars['download_link_extended'] =
			...
			$post_vars['download_link_popup'] = stripcslashes( str_replace( array(
				...
				$loginform,
				__::valueof( $_SERVER, 'REQUEST_URI', [ 'validate' => 'escs' ] ),
				$packurl
			), $loginmsg ) );
			$post_vars['download_link'] =
			$post_vars['download_link_extended'] =
	...
	public function getFiles( $ID, $include_dir = false ) {
		...
		if ( $include_dir ) {
			...
			if ( $package_dir != '' ) {
				...
				if ( $package_dir ) {
					$dfiles = FileSystem::scanDir( $package_dir, true, true, false, true );
					$files  += $dfiles;
				}
			}
		}
	...
	public function metaData( $ID ) {
		...
		if ( is_array( $metaData ) ) {
			foreach ( $metaData as $metaDataRow ) {
				...
				if ( $key === 'publish_date' ) {
					$key                     = 'avail_date';
					$data['avail_date_ts']   = strtotime( $metaDataRow->meta_value );
	...
	function prepareDownloadLink( &$package, $embed = 0, $extras = array() ) {
		...
		if ( isset( $package['expire_date'] ) && $package['expire_date'] != "" && strtotime( $package['expire_date'] ) < time() ) {
			$package['download_url']  = '#';
			$package['link_label']    = apply_filters( "wpdm_download_expired_message", __( "Download was expired on", "download-manager" ) . " " . date_i18n( get_option( 'date_format' ) . " h:i A", strtotime( $package['expire_date'] ) + wpdm_tzoffset() ), $package );
		...
		if ( isset( $package['publish_date'] ) && $package['publish_date'] != '' && strtotime( $package['publish_date'] ) > time() ) {
			$package['download_url']  = '#';
			$package['link_label']    = apply_filters( "wpdm_download_availability_message", __( "Download will be available from ", "download-manager" ) . " " . date_i18n( get_option( 'date_format' ) . " h:i A", strtotime( $package['publish_date'] ) + wpdm_tzoffset() ), $package );
			$package['download_link'] = $vars['download_link_extended'] = $vars['download_link_popup'] = "<a href='#'>{$package['link_label']}</a>";
		...
		if ( ! isset( $package['quota'] ) || ( isset( $package['quota'] ) && $package['quota'] > 0 && $package['quota'] > $package['download_count'] ) || $package['quota'] == 0 ) {
			...
			if ( $sociallock != "" ) {
				$data .= "<div class='panel panel-default card card-default'  id='wpdm-social-locks-{$package['ID']}'><div class='panel-heading card-header'>" . __( "Download", "download-manager" ) . "</div><div class='panel-body card-body wpdm-social-locks text-center'>{$sociallock}</div></div>";
			}
...
//                        $adata .= '<div class="modal fade" role="modal" id="pkg_' . $package['ID'] . "_" . $unqid . '"><div class="modal-dialog"><div class="modal-content"><div class="modal-header"><strong style="margin:0px;font-size:12pt">' . __('Download') . '</strong></div><div class="modal-body">' . $data . '</div><div class="modal-footer text-right"><button type="button" class="btn btn-secondary btn-sm" data-dismiss="modal">Close</button></div></div></div></div>';
	...
	private static function activeLocks( $ID, $params = array( 'embed' => 0, 'popstyle' => 'pop-over' ) ) {
		...
		if ( $sociallock !== "" ) {
			...
			$data    .= "<div class='panel panel-default card' id='wpdm-social-locks-{$ID}'><div class='panel-heading card-header'>" . get_option( '_wpdm_social_lock_panel_title', 'Like or Share to Download' ) . "</div><div class='panel-body card-body wpdm-social-locks text-center'>{$socdesc}{$sociallock}</div></div>";
		}
	 ...
	 * @return string
	 */
	public function downloadLink( $ID, $embed = 0, $extras = [] ) {
		...
		$data = apply_filters( 'wpdm_download_link', $data, $extras + [ 'ID' => $ID, 'id' => $ID ] );
	...
	public function getDownloadURL( $ID = null, $ext = array() ) {
		$ID = $ID ?: $this->ID;
		if ( $this->isLocked( $ID ) && ! Session::get( '__wpdm_unlocked_' . $ID ) ) {
			return '#locked';
		}
		if ( ! is_array( $ext ) ) {
			$ext = [];
	...
	public function fetchTemplate( $template, $vars, $type = 'link' ) {
		...
		if ( is_user_logged_in() && ! self::userCanAccess( $vars['ID'] ) && get_option( '_wpdm_hide_all', 0 ) == 1 ) {
			return $type != 'page' ? "" : Messages::permission_denied( $vars['ID'] );
		}
		...
		if ( $vars['download_link'] == 'blocked' && $type == 'page' ) {
			return Messages::permission_denied( $vars['ID'] );
		}
