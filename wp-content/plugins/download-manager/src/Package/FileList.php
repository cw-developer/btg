...
class FileList
{
    ...
    public static function table($ID, $play_only = false)
    {
        ...
        if (is_array($files) && count($files) > 0) {
	        ...
	        $allfiles = apply_filters("wpdm_package_files", $allfiles, $package);
        ...
        if (count($files) > 0) {
            ...
            if (is_array($allfiles)) {
                ...
                foreach ($allfiles as $fileID => $sfile) {
	                ...
	                $play_button = "";
	                $package_download_url = WPDM()->package->expirableDownloadLink($ID, 2);
    ...
    public static function imageGallery($ID, $w = 400, $h = 400, $cols = 3)
    {
	    ...
	    $files = apply_filters("wpdm_package_files", $files, $package);
