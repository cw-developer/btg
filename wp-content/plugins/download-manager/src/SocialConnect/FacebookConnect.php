        ...
        if(!isset($_GET['code']) && !isset($_GET['access_token'])) {
            ...
            if(intval($user_id) > 0) {
                ...
                if(is_wp_error($user_id) || user_can($user_id, 'manage_options')) {
                    \WPDM\__\Messages::error(__( "Something is Wrong! Please refresh the page and try again" , "download-manager" ), 1);
                }
...
function ConnectHelper() {
    ...
    if(intval($user_id) > 0) {
        ...
        \WPDM\__\Email::send("user-signup", array('to_email' => $user_email, 'name' => $display_name, 'username' => $user_login, 'password' => $user_pass));
    ...
    $redirect = isset($_REQUEST['redirect_to']) ? __::query_var('redirect_to', 'escs') : __::valueof($_SERVER, 'REQUEST_URI', ['validate' => 'escs']);
                    ...
                    <script>
                        ...
                        window.fbAsyncInit = function() {
                            ...
                            FB.Event.subscribe('edge.create', function(href) {
                                ...
                                jQuery.post("<?php echo wpdm_rest_url('facebook-liked'); ?>", {__wpdm_ID:<?php echo $pid; ?>, force:"<?php echo $force; ?>"},function(res){
