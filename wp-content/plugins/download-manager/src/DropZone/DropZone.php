...
class DropZone {
	...
	private function __construct()
	{
		...
		add_action('init', [$this, 'fileRequest']);
	...
	function frontEnd()
	{
		if(!$this->hasAccess()) return Messages::warning(__('DropZone is disabled by admin!', WPDM_TEXT_DOMAIN), -1);
		...
		if(substr_count($_SERVER['REQUEST_URI'], 'wpdm-file-req')) {
			...
			if(!is_user_logged_in()){
				add_action("the_content", function (){
					...
					echo do_shortcode("[wpdm_login_form note_before='{$note}']");
				});
