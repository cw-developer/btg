...
class AuthorDashboard
{
    ...
    function dashboard($params = array())
    {
        ...
        $msg = get_option('__wpdm_front_end_access_blocked', __( "Sorry, You Are Not Allowed!" , "download-manager" ));
        $msg = $msg != ''?$msg:__( "Sorry, You Are Not Allowed!" , "download-manager" );
    ...
    function savePackage()
    {
        ...
        if (isset($_REQUEST['act']) && in_array($_REQUEST['act'], array('_ap_wpdm', '_ep_wpdm')) && count($allowed) > 0) {
	        ...
	        $masterKey = Crypt::encrypt(['id' => $id, 'time' => time()]);

			if(wpdm_query_var('act') === '_ap_wpdm')
				update_post_meta($id, '__wpdm_masterkey', $masterKey);
    ...
    function chunkUploadFile($destFilePath){
        ...
        if (!$chunks || $chunk == $chunks - 1) {
	        ...
	        if(file_exists($destFilePath) && get_option('__wpdm_overwrite_file_frontend',0)==1 ) {
				$dir = dirname($destFilePath);
		        $filename = basename($destFilePath);
				$filename .= time()."wpdm_".$filename;
				$newDestFilePath = trailingslashit( $dir ).$filename;
	        }
            rename("{$destFilePath}.part", $newDestFilePath);
            do_action("wpdm_after_upload_file_frontend", $newDestFilePath);
	        echo "|||".str_replace(UPLOAD_DIR, '', $newDestFilePath)."|||";
			die();
        }
    }
    ...
    function packageForm($params = array()){
        ...
        if(wpdm_query_var('id') > 0 && $current_user->ID != $post->post_author && !current_user_can('manage_options'))
            return "<div class='w3eden'><div class='alert alert-danger'>" . wpautop(stripslashes($msg)) . "</div></div>";
		if(!isset($burl)) $burl = get_permalink();
		if(!isset($sap)) {
			$sap = strpos($burl, '?') ? '&' : '?';
		}
