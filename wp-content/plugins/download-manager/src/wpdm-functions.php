...
function wpdm_thumb($post, $size = '', $echo = true, $extra = null)
{
    ...
    if (is_array($size)) {
        ...
        if ($large_image_url != '') {
            ...
            $img = "<img src='" . $thumb . "' alt='{$alt}' class='{$class}' style='{$style}' />";
            if ($echo) {
                echo $img;
                return;
            } else
...
function wpdm_option_field($data)
{
    ...
    switch ($data['type']):
        ...
        case 'media':
            ...
            break;
        case 'custom':
            return call_user_func($data['callback'], $data);
            break;
        default:
 ...
 * @param $var
 * @param $index
 * @param array $default_or_params
 * @return array|bool|float|int|mixed|string|string[]|null
 */

function wpdm_valueof($var, $index, $default_or_params = [])
{
    return __::valueof($var, $index, $default_or_params);
}
...
add_action("admin_head", function () {
    $pages = ['settings', 'wpdm-asset-manager', 'importable-files', 'wpdm-stats', 'templates', 'wpdm-subscribers', 'wpdm-addons', 'orders', 'pp-license', 'pp-coupon-codes', 'customers', 'payouts', 'wpdm-dropzone'];
