...
if(!defined('WPDM_FONTAWESOME_URL'))
    define('WPDM_FONTAWESOME_URL','https://use.fontawesome.com/releases/v6.1.1/css/all.css');
...
final class WordPressDownloadManager{
    ...
    function enqueueScripts()
    {
        ...
        if(is_object($post) && ( wpdm_query_var('adb_page') != '' || get_option('__wpdm_author_dashboard') == $post->ID || has_shortcode($post->post_content,'wpdm_frontend') || has_shortcode($post->post_content,'wpdm_package_form') || has_shortcode($post->post_content,'wpdm_dropzone') || (function_exists('has_block') && (has_block('download-manager/dashboard') || has_block('download-manager/dropzone'))) || has_shortcode($post->post_content,'wpdm-file-browser') ) ){
